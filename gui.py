...
from config import WINDOW_TITLE, WINDOW_SIZE, OUTPUT_FORMATS, DROPBOX_ACCESS_TOKEN, setup_logging, load_json_config, save_json_config, ALL_FILE_EXTENSIONS, PREFERENCES_FILE, OUTPUTS_DIR
...
class DropboxApp:
    ...
    def browse_output_file(self):
        output_file = filedialog.asksaveasfilename(initialdir=OUTPUTS_DIR, defaultextension=f".{self.output_format.get()}", 
                                                   filetypes=[(f"{self.output_format.get().upper()} files", f"*.{self.output_format.get()}")])
        if output_file:
            self.output_file.set(output_file)
            self.save_preferences()  # Save preferences when the output file is selected

    def set_default_output_file(self):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = f"dropbox_{timestamp}.{self.output_format.get()}"
        default_output_file = os.path.join(OUTPUTS_DIR, default_name)
    ...
    def start_generate_links(self):
        ...
        asyncio.create_task(self.generate_links())
